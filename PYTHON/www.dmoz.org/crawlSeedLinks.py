...
links=[]
seedlinks=[]
website="http://www.dmoz.org/Arts/"
Pool=[]
def seedLinks(code):
	soup = BeautifulSoup(code)
	ul = soup.findAll("a", { "class" : "listinglink" })
	for i in ul:
...
def setProxy():
	import urllib2
	proxy_handler = urllib2.ProxyHandler({'http':'172.16.0.02:8080'})
...
def cleanUp(parent,url):
	...
	if not "http://www.dmoz.org/Arts/" in url:
		return ""
	return url
