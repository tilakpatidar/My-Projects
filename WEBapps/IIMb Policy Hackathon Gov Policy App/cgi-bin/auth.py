...
def newSession(sid,username):
	try:
		...
		db1 = MySQLdb.connect("localhost","root","1","iimb" )
	...
	if pswd==password:
		...
		if string_cookie:
			if username=="admin":
				...
				print  "<SCRIPT type='text/javascript'>window.location.href='/admin';</SCRIPT>"
			else:
				print "<SCRIPT type='text/javascript'>window.location.href='/vote';</SCRIPT>"
		else:
