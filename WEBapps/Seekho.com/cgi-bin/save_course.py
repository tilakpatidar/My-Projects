...
def updateFile(k):
	...
	try:
		...
		cursor1 = db1.cursor()
		# execute SQL query using execute() method.
		sql1 = "INSERT INTO file (`file_title`, `file_desc`, `file_link`, `file_author`) VALUES ('%s', '%s','%s', '%s');"%(escape_string(d['title']),escape_string(d['description']),escape_string(d['link']),author)
		# Execute the SQL command
		sql2 = "SELECT LAST_INSERT_ID();"
		cursor1.execute(sql1)
		cursor1.execute(sql2)
