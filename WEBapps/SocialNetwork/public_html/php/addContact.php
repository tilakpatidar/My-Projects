...
if (isset($_GET['term'])){
	...
	try {
	    ...
	    while($row = $stmt->fetch()) {
	        $return_arr[] ='<!'.$row['username'].'><div style="float:left;">'.'<img src="/images/'.$row['profile_pic'].'" class="image" width=40 height=35></img></div><label><div style="float:right"><div style="float:top;font-size:larger;  font-style:normal; font-weight:lighter;margin-left:20px;">'.$row['CONCAT(fname,CONCAT(CONCAT(" ",lname)))'].'</div><div style="font-size:small;  font-style:normal; font-weight:lighter;margin-left:20px;float:bottom;">Vertical : '.$row['vertical'].'</label></div></div>';
	    }
