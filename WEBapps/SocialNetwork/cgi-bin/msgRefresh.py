...
def showMessages(username,msg_from):
	...
	try:
		...
		cursor2 = db2.cursor()
		# execute SQL query using execute() method.
		sql2="SELECT msg_from,message,time,seen from messages where (msg_to='%s' AND msg_from='%s') OR (msg_to='%s' AND msg_from='%s') ORDER BY time"%(username,msg_from,msg_from,username)
...
msg_from  =form.getvalue('msg_from')
